...
bn_running_std = torch.ones((1,l1_size))
...
for step in range(step_count):
    ...
    h = ((emb_flattened @ W1) + B1).tanh()
    bn_h = bn_gain * (h-h.mean(dim=0, keepdim=True))/h.std(dim=0, keepdim=True) + bn_bias
    logits = bn_h @ W2 + B2
    loss = torch.nn.functional.cross_entropy(logits, ytr[idx])

    with torch.no_grad():
        bn_running_mean = (1-m) * bn_running_mean + m * h.mean(dim=0, keepdim=True)
        bn_running_std = (1-m) * bn_running_std + m * h.std(dim=0, keepdim=True)
        h = ((emb_flattened_dev @ W1) + B1).tanh()
        bn_h = bn_gain * (h-h.mean(dim=0, keepdim=True))/h.std(dim=0, keepdim=True) + bn_bias
        logits_dev = bn_h @ W2 + B2
...
h = ((embcat_test @ W1) + B1).tanh()
bn_h = bn_gain * (h-bn_running_mean)/bn_running_std + bn_bias
logits_test= bn_h @ W2 + B2
